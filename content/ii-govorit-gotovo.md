...
На первой сверке обнаружилось главное расхождение: агент несколько дней сообщал о выполненной работе, но Notion оставался почти пустым. Затем появились другие несоответствия. В отчётах он описывал распределение задач между Claude и Gemini, а журналы показывали только Gemini. В расчёте возникла стоимость внешней расшифровки, хотя на сервере уже был установлен Whisper.
...
## Разбор случая: отчёт был, внешнего результата не было
...
На первой контрольной точке я сверила отчёты агента с содержимым Notion.
...
2. **Намерение.** Агент собирался выполнить поручение.
